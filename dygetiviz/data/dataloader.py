...
def load_data() -> dict:
    ...
    if isinstance(config['reference_nodes'], str) and config['reference_nodes'].endswith("json"):
        reference_nodes = json.load(open(osp.join("data", args.dataset_name, config['reference_nodes']), 'r', encoding='utf-8'))
    ...
    if isinstance(config['projected_nodes'], str) and config['projected_nodes'].endswith("json"):
        projected_nodes = json.load(open(osp.join("data", args.dataset_name, config['projected_nodes']), 'r', encoding='utf-8'))
    ...
    if node_presence is None:
        node_presence = np.load(osp.join("data", args.dataset_name, "node_presence.npy"))
