...
def train():
    ...
    snapshot_indices = torch.linspace(0, len(train_loader), args.num_snapshots, dtype=int)
    idx_snapshot = 0


    embeddings = torch.empty((data.num_nodes, args.embedding_dim), dtype=torch.float32, device=device_viz)

    # Whether the node has appeared in the current snapshot.
    node_presence = torch.zeros((data.num_nodes, args.num_snapshots), dtype=torch.bool, device=device_viz)
    ...
    for idx_batch, batch in enumerate(train_loader):
        ...
        # Store the up-to-date node embeddings to the snapshot.
    ...
    node_presence_li = np.stack(node_presence_li, axis=0)


    return total_loss / train_data.num_events, embeds_li, node_presence_li
...
def test(loader, neg_sampler, split_mode):
    ...
    for pos_batch in tqdm(loader, desc="Evaluating", position=0, leave=True):
...
tgb_args = argparse.Namespace(**json.load(open(osp.join("config", 'TGBL.json'))))
...
for run_idx in range(NUM_RUNS):
    ...
    for epoch in trange(1, NUM_EPOCH + 1, desc="Train"):
        ...
        loss, embeds_li, node_presence_li = train()
        print(
        ...
        if epoch % args.save_embeds_every == 0:
            ...
            print(f"[Embeds] Saving embeddings for Ep. {epoch} ...", end=" ")
            np.save(osp.join(args.data_dir, args.dataset_name,
                    ...
                    embeds_li)
            np.save(osp.join(args.data_dir, "embeds", "node_presence.npy"), node_presence_li)
